...
def load_configuration(configuration_file):
    ...
    f_tmp = file(configuration_file, 'r')
    global PARAMETERS
...
def get_time(star_evt=0, end_evt=0, total_time=0):
    ...
    tm_mn, tm_sc = divmod(total_time, 60)
    tm_hr, tm_mn = divmod(tm_mn, 60)

    return "%i:%i:%i"%(tm_hr, tm_mn, tm_sc)
...
def make_sbatch_file(in_file, star_evt, end_evt, sub_idx):

    base_name = get_basename(in_file)
    ...
    sbatch_file.write("srun -n 1 shifter /bin/csh -c \"source /usr/local/star/group/templates/cshrc; root4star -l -b -q "+
                      "'bfc.C(%i,%i,\\\"%s\\\",\\\"%s\\\")'\""%(star_evt, end_evt, PARAMETERS['CHAIN'], in_file))
    sbatch_file.write('\n')
...
def process_file(in_file):
    total_time = in_file[1] * PARAMETERS['CPU_PER_EVENT']
    number_jobs = int(math.ceil(float(total_time)/float(PARAMETERS['QUEUE_LIMIT'])))
    events_per_job = in_file[1] / number_jobs

    base_name = get_basename(in_file[0])
    log_yaml = {}
    log_yaml['file'] = '%s.daq'%base_name
    log_yaml['number_events'] = '%i'%in_file[1]
    ...
    global JOB_IDX
    for j_idx in xrange(0, number_jobs):
        start_evt = 1 + j_idx * events_per_job
        end_evt = (j_idx+1) * events_per_job
        JOB_IDX += 1
        sub_file = '%s_%i:%i:%i'%(SUBMISSION_ID, JOB_IDX, start_evt, end_evt)

        sbatch_filename = make_sbatch_file(in_file[0], start_evt, end_evt, j_idx)
        if SUBMIT:
